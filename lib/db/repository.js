...
exports.list = (org, orderBy = 'name', desc = false, limit = 25, offset = 0) => {
  const orderByFields = ['name', 'updated']
  let orderByField = orderBy
  let order = desc
  if (!orderByFields.includes(orderByField)) {
  ...
  return pool.query(`SELECT COUNT(*) OVER() AS total, *
    FROM repositories
    WHERE org = $1
    ORDER BY ${orderByField} ${(order) ? 'DESC' : ''}
    LIMIT $2 OFFSET $3`, [org, limit, Math.abs(offset)]
  )
}
